...
export function Composer({ state, onWrite }: { state: GameState; onWrite: () => void }) {
  ...
  return (
    <main className="composer">
      ...
      <div className="mail-fields"><div><span>Da:</span><strong>segreteria@ordinedelleonde.example</strong></div><div><span>A:</span><mark>{contact.firstName} {contact.lastName} &lt;{contact.email}&gt;</mark></div><div><span>Oggetto:</span><strong>{email.subject}</strong></div></div>
      <div
        className="mail-body"
        role="button"
        tabIndex={0}
