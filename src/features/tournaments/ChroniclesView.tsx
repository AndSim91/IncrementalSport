...
const CHRONICLES_ATHLETES_PER_PAGE = 10;
  ...
  return (
    <section className="chronicles-duel" aria-labelledby="chronicles-duel-title">
      <div className="chronicles-duel-heading">
        <div className="chronicles-monogram" aria-hidden="true">
          <span>{initial}</span>
        </div>
        <div>
      ...
      <div
        className="chronicles-score"
        aria-label={`Punteggio: tu ${challenge.playerWins}, ${profile.firstName} ${challenge.legendaryWins}`}
      >
        <span>
          <small>TU</small>
          <strong>{challenge.playerWins}</strong>
        </span>
        <i aria-hidden="true" />
        <span>
          <small>{legendaryLabel}</small>
          <strong>{challenge.legendaryWins}</strong>
        </span>
        <em>
          {challenge.playerWins === 1 && challenge.legendaryWins === 1
            ? "Mano decisiva"
            : "Primo a 2"}
        </em>
      </div>
          ...
          <div>
            {(["paper", "scissors", "rock"] as const).map((choice) => (
              <button
                key={choice}
                type="button"
                onClick={() => onPlayHand(choice)}
                aria-label={`Gioca ${CHOICE_LABELS[choice]}`}
              >
...
export function ChroniclesView({ state, onStartTournament, onPlayHand }: ChroniclesViewProps) {
  ...
  const preparationByContactId = useMemo(
    () =>
      new Map(
    ...
    [collaboratorsByContactId, eligible],
  );
  const sortedEligible = useMemo(
    () =>
      [...eligible].sort((left, right) => {
        const leftPreparation = preparationByContactId.get(left.id)!;
        const rightPreparation = preparationByContactId.get(right.id)!;
  ...
  const firstAthlete = page * CHRONICLES_ATHLETES_PER_PAGE;
  const visibleEligible = sortedEligible.slice(
    firstAthlete,
    firstAthlete + CHRONICLES_ATHLETES_PER_PAGE,
  );
  ...
  if (!chronicles.unlocked) {
    return (
      <section className="chronicles-locked" aria-labelledby="chronicles-locked-title">
        ...
        <p>
          Vinci Arena e Stile nella stessa edizione della Champion&apos;s Arena per ottenere la
          prima chiave.
        </p>
      </section>
    );
  ...
  const toggleSelection = (contactId: string) => {
    setSelectedIds((current) => {
      ...
      return valid.includes(contactId)
        ...
        : valid.length < GAME_CONFIG.chroniclesTeamSize
          ? [...valid, contactId]
          : valid;
    });
  };
  const preparationFor = (contact: GameState["contacts"][number]) => {
    return preparationByContactId.get(contact.id)!;
  };
  ...
  return (
    <div className="chronicles-lobby">
      <header className="chronicles-banner">
        <span className="chronicles-key-emblem">
          <ChroniclesKeyIcon />
        </span>
        <div>
          <h2>Chronicles of Ludosport</h2>
          <strong>
            {chronicles.keys} {chronicles.keys === 1 ? "chiave disponibile" : "chiavi disponibili"}
          </strong>
      ...
      <div className="chronicles-selection">
        <section className="chronicles-available" aria-labelledby="chronicles-available-title">
          ...
          <div className="chronicles-roster-list">
            {visibleEligible.map((contact) => {
              ...
              return (
                <label
                  key={contact.id}
                  className={checked ? "is-selected" : full ? "is-disabled" : ""}
                >
                  <input
                    type="checkbox"
                    checked={checked}
          ...
          {pageCount > 1 ? (
            <nav
              className="chronicles-pagination list-pagination"
              aria-label="Pagine atleti disponibili"
            >
              <button
                type="button"
                disabled={page === 0}
        ...
        <section className="chronicles-selected" aria-labelledby="chronicles-selected-title">
          <header>
            ...
            <span>
              {selected.length} / {GAME_CONFIG.chroniclesTeamSize}
            </span>
          </header>
          <div className="chronicles-selected-head">
            <span>#</span>
            <span>Atleta</span>
            <span>Arena</span>
            <span>Stile</span>
              ...
              return (
                <li key={contact.id}>
                  ...
                  <button
                    ...
                    aria-label={`Rimuovi ${contact.firstName} ${contact.lastName}`}
                  >
                    ×
                  </button>
                </li>
          ...
          <footer>
            ...
            <button
              ...
              className="primary"
              disabled={!canStart}
              onClick={() => onStartTournament(activeSelectedIds)}
            >
              <ChroniclesKeyIcon /> Avvia le Chronicles
            </button>
          </footer>
          {chronicles.keys === 0 ? (
            <p className="chronicles-no-key">
              Conquista un’altra chiave alla Champion&apos;s Arena.
            </p>
          ) : null}
        </section>
      </div>
