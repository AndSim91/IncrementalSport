...
export function useGameEngine() {
  ...
  const saveCheckpoint = `${state.profile.displayName}:${state.contacts.length}:${state.school.activeMembers}:${state.school.historicMembers}:${state.school.euros}:${state.statistics.emailsSent}:${state.statistics.trialsBooked}:${state.statistics.membersEnrolled}:${state.statistics.eventsCompleted}:${JSON.stringify(state.upgrades)}:${state.messages.filter((message) => message.unread).length}:${state.acquisitionEvents.length}`;
  useEffect(() => {
    saveGame(stateRef.current);
  }, [saveCheckpoint]);
