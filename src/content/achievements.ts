...
export const ACHIEVEMENTS: AchievementDefinition[] = [
  ...
  { id: "no-recognizable-reference", title: "Nessun riferimento legalmente riconoscibile", description: "Venti eventi a tema gestiti con impeccabile prudenza narrativa.", euroReward: 20, condition: (state) => getArchivedCompletedEventCount(state.historyArchive, "themed-event") + state.acquisitionEvents.filter((event) => event.definitionId === "themed-event" && event.status === "completed").length >= 20 },
];
